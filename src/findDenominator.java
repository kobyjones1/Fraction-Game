...
public class findDenominator implements ActionListener{
	...
	public findDenominator(int intQuesDen1, int intQuesDen2) {
		...
		JLabel lblTip1 = new JLabel(strLCD1);	//Displays directions for obtaining the LCD.
		...
		txtUserAnswer = new JTextField();	//Responsible for obtaining the user's input.
		txtUserAnswer.setBounds(198, 150, 75, 25);
		pnlFindDen.add(txtUserAnswer);

		SwingUtilities.invokeLater(new Runnable() {	//Places the focus (blinking cursor) on the txtUserAnswer. 
			public void run() { 
		...
		lblStatus = new JLabel("");	//Tells the user if their answer is incorrect or if input was not detected.
	...
	private static void close(int intLCD) {	//Prepares the startGame window as findDenominator closes.
	...
	private void checkAnswer(int intLCD, String strLCD) {	//Checks if the player's answer is correct.
		...
		if(txtUserAnswer.getText().equals(strLCD)) {	//Closes findDenominator if the user's answer is correct.
			startGame.addScore(65);	//Increase the score by 60 (+5 bonus points).
			frmFindDen.dispose();
			close(intLCD);
		}
			...
			startGame.subScore(25);	//Decrease the score by 15 (+10 bonus points).
			...
			txtUserAnswer.setBackground(Color.red);
			lblStatus.setText(txtUserAnswer.getText() + " is incorrect.");	//Displays if they player is spamming the submit button.
		}
	}
	...
	public void actionPerformed(ActionEvent e) {
		if(e.getSource() == btnSubmit) {
			...
			checkAnswer(intLCD, strLCD);	//Checks if the player's answer was correct.
		}	
